...
//    goToTxt.onclick = hideShare;
    ...
    var pageIter = (pageCount%2==0)? pageCount/2 : (pageCount/2)+1 ; 
    var values = "";
...
function updateMethod(method){
    ...
    if(method=="plus"){        
        ...
        // rigthDiv.style.backgroundColor="white";
        }
        window.setTimeout(function(){
            if((curRP+1)>=1 && (curRP+1)<=pageCount){
                //commente out code for load
                // d('page'+(curRP+1)).innerHTML = "";
                // exchange(leftDiv,d('page'+(curRP-1)));
                leftDiv.innerHTML ="";
                exchange(dragContent,leftDiv);
            // leftDiv.innerHTML = dragContent.innerHTML;
            // getPage(leftDiv,curRP-1);
            }else{
                rightDiv.innerHTML = "";
                rightDiv.style.backgroundColor=boardColor;
                //commente out code for load
                // exchange(leftDiv,d('page'+(curRP)));
                leftDiv.innerHTML = "";
                leftDiv.style.backgroundColor=boardColor;
            }
            if((curRP+2)>=1 && (curRP+2)<=pageCount){                
                // //commente out code for load
                // d('page'+(curRP+2)).innerHTML = "";
                // exchange(rightDiv,d('page'+(curRP)));
                rightDiv.innerHTML = "";
                exchange(clipContent,rightDiv);
            // rightDiv.innerHTML = clipContent.innerHTML;
            ...
            if(isEC){
                removeVideoLinks();
                if(curRP == 25) {
                    addVideoLink(500,200, 680, 368, document.getElementById("container"), "http://www.youtube.com/embed/YvVPjZ-wvkE");
                }
                else if(curRP == 27) {
                    addVideoLink(500,200, 680, 368, document.getElementById("container"), "http://www.youtube.com/embed/ZdackF2H7Qc");
                }
            }            
        }, 1);
    }else{
        ...
        window.setTimeout(function(){            
            ...
            //commente out code for load
            // if(d('page'+(curRP-3))!=null){
            // d('page'+(curRP-3)).innerHTML = "";
            // }
            // if(d('page'+(curRP-2))!=null){
            // d('page'+(curRP-2)).innerHTML = "";
            // }
            makeInvisible(clipElement,false);
            makeInvisible(dragElement,false);
...
function addVideoLink(x, y, width, height, container, link) {
    ...
    linkEl.setAttribute("id", "videoLink");
    ...
    container.insertBefore(linkEl,bookDiv);
}
